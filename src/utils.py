...
def print_and_log(string, filename):
    """ Prints and saves string to log file """
    
    print(string)
    with open(filename, 'a') as write_file:
        write_file.write(string + '\n')
        
def classwise_f1(predictions, targets, idx_to_label=None):
    """ Calculates class-wise F1 scores """
    ...
    for c in set(targets):
        ...
        f1[key] = f1_score(
            (targets == c).astype(np.int), (predictions == c).astype(np.int)
        )
    return f1
